...
export class ConstructorsFavoriteService {
  ...
  create(constructorId: string): Constructor {
    const result = this._constructorsFavorite.find(
      (element) => element.constructorId === constructorId,
    );
    ...
    if (result === undefined) {
      ...
      throw new ConflictException(
        `Le constructeur avec l'id '${constructorId}' est déjà ajouté aux favoris`,
      );
    }
  }
  ...
  delete(constructorId: string): Constructor {
    ...
    if (result !== undefined) {
      const constructorIndex = this._constructorsFavorite.findIndex(
        (element) => element.constructorId === constructorId,
      );
      ...
      throw new NotFoundException(
        `Le constructeur avec l'id '${constructorId}' n'existe pas`,
      );
    }
  }
}
